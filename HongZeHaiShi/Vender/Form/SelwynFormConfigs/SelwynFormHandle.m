...
CGFloat const EdgeMarin = 5.0f;
CGFloat const TitleWidth = 80.0f;
...
+ (SelwynFormItem *)checkContentWithItem:(SelwynFormItem *)item{
    ...
    if (item.required) {
        ...
        if (!item.formDetail || [item.formDetail isEqualToString:@""]) {
            ...
            if (item.formCellType == SelwynFormCellTypeInput || item.formCellType == SelwynFormCellTypeTextViewInput || item.formCellType == SelwynFormCellTypeInputAndImage) {
