...
#import <lame/lame.h>
#import "ZLUtility.h"
@interface ZLAudioManager()<AVAudioRecorderDelegate,AVAudioRecorderDelegate,AVAudioPlayerDelegate>{
...
@implementation ZLAudioManager

+ (instancetype)shareManager{
    
    static ZLAudioManager *manager = nil;
...
- (void)beginRecord{
    ...
    filePath = [ZLUtility getAudioFilePath];
    ...
    if (error) {
        ...
        UIAlertView *alertView = [[UIAlertView alloc]initWithTitle:@"错误提示" message:@"录音失败" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
        [alertView show];
        return;
    }
...
- (NSString *)audioToMP3:(NSString *)cafPath
{
    ...
    @catch (NSException *exception) {
        ZLLog(@"音频转码异常：%@",[exception description]);
    }
    @finally {
