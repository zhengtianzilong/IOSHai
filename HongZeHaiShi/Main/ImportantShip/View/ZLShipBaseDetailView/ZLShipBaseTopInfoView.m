...
- (ZLShipInfoView *)shipName{
    if (!_shipName) {
        _shipName = [[ZLShipInfoView alloc]initWithTitle:@"船名:" imageName:@"" fontSize:15];
    }
    return _shipName;
}
...
- (ZLShipInfoView *)shipOwner{
    if (!_shipOwner) {
        _shipOwner = [[ZLShipInfoView alloc]initWithTitle:@"船主:" imageName:@"" fontSize:15];
    }
    return _shipOwner;
}
...
- (ZLShipInfoView *)shipOperator{
    if (!_shipOperator) {
        _shipOperator = [[ZLShipInfoView alloc]initWithTitle:@"经营人:" imageName:@"" fontSize:15];
    }
...
- (ZLShipInfoView *)telephone{
    if (!_telephone) {
        _telephone = [[ZLShipInfoView alloc]initWithTitle:@"电话:" imageName:@"" fontSize:15];
    }
    return _telephone;
}
...
- (ZLShipInfoView *)deadWeight{
    if (!_deadWeight) {
        _deadWeight = [[ZLShipInfoView alloc]initWithTitle:@"载重吨:" imageName:@"" fontSize:15];
    }
    return _deadWeight;
}
...
- (ZLShipInfoView *)weight{
    if (!_weight) {
        _weight = [[ZLShipInfoView alloc]initWithTitle:@"总吨位:" imageName:nil fontSize:15];
    }
    return _weight;
}
...
- (ZLShipInfoView *)lengthAndWidth{
    if (!_lengthAndWidth) {
        _lengthAndWidth = [[ZLShipInfoView alloc]initWithTitle:@"长、宽:" imageName:@"" fontSize:15];
    }
...
- (ZLShipInfoView *)people{
    if (!_people) {
        _people = [[ZLShipInfoView alloc]initWithTitle:@"配员:" imageName:nil fontSize:15];
    }
    return _people;
}
